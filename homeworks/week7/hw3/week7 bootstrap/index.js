...
$(document).ready(function() {
    ...
    */
    ...
    $('.container').on('click', 'input.submit__btn', function(e) {
        ...
        console.log($eT.parents('.comments__wrapper').find('.nick').text().trim())
        ...
        $.ajax({
            ...
            if(resp.result === 'Success') { // server 成功才執行
                if(parentId === '0' && !checkSubSubmit) { // 檢查 id 及 判斷 submit
                    ...
                    clearContent
                    $eT.parents('.meg__wrap').find('.comment__content').after(createSubComment(showNickname, getContent, id, false))
                    $('.comment__content + .sub-meg__user').hide().show(500)
                }
            } else {
                // alert('4')
        ...
        if(flag) {
            return `
                <div class="sub-meg__user alert alert-light mb-2"> 
                    ...
                    <div class="sub-comment__content card-body">${getContent}</div>
                </div>`;
        } else {
            return `
