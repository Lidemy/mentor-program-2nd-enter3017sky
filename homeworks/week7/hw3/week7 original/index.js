...
$(document).ready(function() {
    ...
    */
        ...
        $.ajax({
            ...
            if(resp.result === 'Success') { // server 成功才執行
                if(parentId === '0' && !checkSubSubmit) { // 檢查 id 及 判斷 submit
                    ...
                    clearContent
                    $eT.parents('.meg__wrap').find('.comment__content').after(createSubComment(showNickname, getContent, id))
                    $('.comment__content + .sub-meg__user').hide().show(500)
    ...
    $('.container').on('click', '.edit__submit', function(e) {
        ...
        console.log(
            'checkOnlyMsg.length  ', checkOnlyMsg.length,
            "$eT.parents('.meg__wrap').length ", $eT.parents('.meg__wrap').length,
            'sub-meg__user meg__wrap.length: ', checkSubEdit
        )
    ...
    function createMainComment(showNickname, getContent, id) {
            ...
            <div class="meg__wrap"> 
                <div class="comment__header">
                    <div class="comment__header__left">
                        <div class="comment__author">${showNickname}</div> 
                    ...
                    <div class="comment__header__right">
                        <div class='delete__comment'>
                            <input type='button' value='刪除' class='delete__btn' data-id='${id}' />
                        </div>
                        <div class='edit__comment'>
                            <input type='button' value='編輯' class='edit__btn' data-id='${id}' />
                        </div>
                    </div>
                </div>
                <div class="comment__content">${getContent}</div>
        ...
        // 新增子留言
    function createSubComment(showNickname, getContent, id) {
        return `
        <div class="sub-meg__user">
            ...
            <div class="sub-comment__content">${getContent}</div>
        </div>`;
     }
    ...
    function editMainBox(elem, content, editId) {
        ...
        $('<input>', {
            class: 'submit__btn edit__submit',
            type: 'submit',
            value: '更新留言'
